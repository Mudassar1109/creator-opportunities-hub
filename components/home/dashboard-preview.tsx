...
export function DashboardPreview({ opportunities, stats }: DashboardPreviewProps) {
  ...
  return (
    <div className="relative">
      {/* Outer glow ring */}
      <div className="absolute -inset-4 rounded-3xl bg-gradient-to-r from-indigo-600/20 via-cyan-500/20 to-indigo-600/20 blur-2xl" aria-hidden="true" />

      {/* Main dashboard window */}
      <div className="relative overflow-hidden rounded-2xl border border-white/20 bg-slate-900/95 shadow-2xl shadow-indigo-900/30">
        {/* Scan line effect */}
        ...
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
          ...
          <div className="flex items-center gap-2 rounded-md bg-white/10 px-3 py-1">
            ...
            <span className="text-[10px] font-medium text-white/50">dashboard.creatorhub.io</span>
          </div>
        ...
        <div className="grid grid-cols-4 gap-px bg-white/5 px-0">
          {[
            { label: "Opportunities", value: stats.publishedOpportunities, color: "text-indigo-400" },
            { label: "Creators", value: stats.totalCreators, color: "text-cyan-400" },
            { label: "Brands", value: stats.totalBrands, color: "text-blue-400" },
            { label: "Applications", value: stats.totalApplications, color: "text-emerald-400" },
          ].map((s) => (
            <div key={s.label} className="bg-slate-900/90 px-3 py-2.5 text-center">
              <p className={`text-sm font-bold tabular-nums ${s.color}`}>
                {s.value > 1000 ? `${(s.value / 1000).toFixed(1)}K` : s.value}
              </p>
              <p className="mt-0.5 text-[9px] font-medium uppercase tracking-wide text-slate-500">{s.label}</p>
            </div>
          ))}
        </div>
        ...
        <div className="p-4">
          <div className="mb-3 flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-400">Recent Opportunities</span>
              ...
              topOpps.map((opp, i) => (
                ...
                >
                  <div className="flex min-w-0 items-center gap-2.5">
                    <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-600/60 to-cyan-500/60 text-xs font-bold text-white">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-[10px] text-slate-500">{opp.brand_name}</p>
                    </div>
                  </div>
                  <div className="flex shrink-0 flex-col items-end gap-1">
                    <span className={`rounded px-1.5 py-0.5 text-[9px] font-bold uppercase ${TYPE_COLORS[opp.opportunity_type] ?? "bg-slate-100 text-slate-600"}`}>
        ...
        <div className="border-t border-white/10 px-4 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <div className="h-6 w-6 rounded-full bg-gradient-to-br from-indigo-500 to-cyan-500" aria-hidden="true" />
              <span className="text-xs font-medium text-slate-400">Your Dashboard</span>
            </div>
            <div className="flex items-center gap-1.5 rounded-lg bg-indigo-600/80 px-3 py-1.5">
      ...
      >
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className="text-xs font-bold text-slate-900">Application Accepted</p>
            <p className="text-[10px] text-slate-500">Brand Deal opportunity</p>
          </div>
        </div>
      </div>
      ...
      <div
        className="animate-float-slow absolute -left-6 bottom-12 z-10 hidden rounded-xl border border-indigo-200/70 bg-white/96 p-3 shadow-xl shadow-indigo-500/10 backdrop-blur-sm xl:block"
        aria-hidden="true"
      >
        <div className="flex items-center gap-2.5">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-indigo-500 to-cyan-500 text-sm shadow-sm">
            🚀
          </div>
          <div>
            <p className="text-xs font-bold text-slate-900">New opportunity posted</p>
            <p className="text-[10px] text-slate-500">Sponsorship · Just now</p>
          </div>
        </div>
      </div>
    </div>
