...
export default async function AdminApplicationsPage({ searchParams }: Props) {
  ...
  const { applications, total, page: currentPage, totalPages } = await getAdminApplications({
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <div className="mb-6">
        <h1 className="text-2xl font-extrabold text-gray-900 dark:text-gray-100">Applications</h1>
        <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{total} application{total !== 1 ? "s" : ""} on the platform</p>
      </div>

      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <form method="GET" action="/admin/applications" className="flex-1 sm:max-w-xs">
